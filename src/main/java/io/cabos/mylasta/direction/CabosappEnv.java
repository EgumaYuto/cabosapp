...
public interface CabosappEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/cabosapp */
    ...
    /** The key of the configuration. e.g. returnpath@cabos.io */
    String MAIL_RETURN_PATH = "mail.return.path";

    /** The key of the configuration. e.g. cabosapp-support@annie.example.com */
    String MAIL_ADDRESS_SUPPORT = "mail.address.support";

    /** The key of the configuration. e.g. org.h2.Driver */
    String JDBC_DRIVER = "jdbc.driver";

    /** The key of the configuration. e.g. jdbc:h2:file:$classes(io.cabos.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/cabosappdb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. cabosappdb */
    ...
    /** The key of the configuration. e.g. localhost:8090/cabosapp */
     ...
     * The value is, e.g. /tmp/lastaflute/cabosapp <br>
     ...
     * The value is, e.g. returnpath@cabos.io <br>
     ...
     * The value is, e.g. cabosapp-support@annie.example.com <br>
     * comment: Mail Address for Cabosapp Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * The value is, e.g. jdbc:h2:file:$classes(io.cabos.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/cabosappdb <br>
     ...
     * The value is, e.g. cabosappdb <br>
     ...
     * The value is, e.g. localhost:8090/cabosapp <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements CabosappEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(CabosappEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(CabosappEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(CabosappEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(CabosappEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(CabosappEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(CabosappEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(CabosappEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(CabosappEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(CabosappEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(CabosappEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(CabosappEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(CabosappEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(CabosappEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(CabosappEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(CabosappEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(CabosappEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(CabosappEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(CabosappEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(CabosappEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(CabosappEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(CabosappEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(CabosappEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(CabosappEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(CabosappEnv.SERVER_DOMAIN);
        }
    }
}
